...
from .fixtures.transactions import tangerine_txns
...
def test_transactions_get___tangerine_account(tangerine_txns):
    response = requests.get('http://localhost:7000/api/accounts/4')
    assert response.status_code == 200
    response_json = response.json()
    assert len(tangerine_txns) == len(response_json['transactions'])
    assert response_json['current_page'] == 1
    assert response_json['total_pages'] == 1


def test_transactions_get___pagination(tangerine_txns):
...
def test_transactions_get___pagination___edge_case(tangerine_txns):
